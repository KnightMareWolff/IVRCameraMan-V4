...
{
	Film       = 0 UMETA(DisplayName = "Record for Film"),
	TV         = 1 UMETA(DisplayName = "Record for TV"),
	Game       = 2 UMETA(DisplayName = "Record for Games"),
	SlowMotion = 3 UMETA(DisplayName = "Record for SlowMotion")
};
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "IVR|Effects|Parameters")
...
public:
	...
	*/
	UFUNCTION(Category = "IVR|Video|Functions", BlueprintCallable)
	...
	*/
	UFUNCTION(Category = "IVR|Video|Functions", BlueprintCallable)
	static int IVR_GetNewRecorderID()
	{
		pIVR_Recorders++;
		return pIVR_Recorders;
	}

	/**
	* This function will clean up all the camera pool and prepare the scene for a new recording.
	*/
	UFUNCTION(Category = "IVR|Settings|Functions", BlueprintCallable)
